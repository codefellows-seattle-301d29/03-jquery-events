...
articleView.populateFilters = function() {
  $('article').each(function() {
    ...
    if (!$(this).hasClass('template')) {
      ...
      // To so, Build an <option> DOM element that we can append to the author <select> element.
      ...
      // TOD: Refactor this concatenation using a template literal.
...
articleView.handleAuthorFilter = function() {
  $('#author-filter').on('change', function() {
    ...
    if ($(this).val()) {
      // TOD: If the <select> menu was changed to an option that has a value, we first need to hide all the articles, and then show just the ones that match for the author that was selected.
      // Use an "attribute selector"   (!!!!!!! $('.active[id="turtle"])!!!!!!!) to find those articles, and fade (!!!!! use jQuery fade in!!!!!) them in for the reader.
      $('option').hide();
...
articleView.handleCategoryFilter = function() {
  ...
  // Be sure to reset the #author-filter while you are at it!   (!!!!!checkout yesterdays demo!!!!!!)
...
articleView.setTeasers = function() {
  ...
  // TODO: Add an event handler to reveal all the hidden elements, when the .read-on link is clicked. You can go ahead and hide the "Read On" link once it has been clicked. Be sure to prevent the default link-click action!   (!!!!!try to addd the read-less??!!!!!!!!)
